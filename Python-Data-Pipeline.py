...
def preprocess_csv(input_csv_path, mapping_file_path):
    ...
    # Change the date format to MMDDYYYY
    df['MemberDOB'] = df['MemberDOB'].dt.strftime('%m%d%Y')
    # Replace missing dates with a fixed date (e.g., '01012000' for January 1, 2000)
    fixed_date = '01012000'
    df['MemberDOB'] = df['MemberDOB'].fillna(fixed_date)
    print(df['MemberDOB'])
